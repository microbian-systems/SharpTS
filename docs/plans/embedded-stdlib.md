...
- ~~URLSearchParams stores keys and values as two parallel `string[]` arrays, not a `string[][]` of pairs — nested-array push on class fields still has a codegen gap where the inner array gets wrapped one level deep.~~ ✅ Fixed 2026-04-18: The codegen gap had silently been resolved by some prior fix; verified by refactoring `URLSearchParams` to a single `_pairs: string[][]` field. All 80 URL tests pass; the workaround comment was stale.
- ~~URLSearchParams.sort uses insertion sort, not `Array.sort` with a comparator, because `this` captures inside arrow comparators fail to resolve.~~ ✅ Fixed 2026-04-18: The actual root cause was arrow-parameter type inference from expected callback signatures, not `this` capture. When an arrow is passed where a function type is expected, `TypeChecker.Calls.cs` now calls `CheckArrowFunction(arrow, expectedType)` so untyped params flow through contextual typing. URLSearchParams.sort now uses `Array.sort((a, b) => ...)` with `this` capturing lexically.
- ~~`get`/`set`/`delete` are assigned as per-instance function properties in the constructor because the parser treats `get`/`set` as accessor keywords and `delete` as a reserved word when used as class method names.~~ ✅ Fixed 2026-04-18: Parser now treats `get`/`set` as method/field names when followed by `(`, `<`, `:`, `=`, `;`, `?`, `!` (lookahead disambiguation); class `MethodDefinition` accepts any PropertyName per ES2015+ spec, including reserved words like `delete`. URLSearchParams now declares these as normal class methods.
- ~~String comparison via `localeCompare` instead of `>` because compiled-mode relational operators on strings coerce to numbers today.~~ ✅ Fixed 2026-04-18: TypeChecker allows `string <op> string`; Interpreter and ILEmitter dispatch to `string.CompareOrdinal` for JS AbstractRelationalComparison semantics. URLSearchParams.sort now uses `>` directly.
...
### Phase 3o — `readline` migration ✅ COMPLETE

Migrated `readline` to `stdlib/node/readline.ts` via a new `primitive:readline`. Second use of the class-instance-via-primitive pattern (async_hooks was the first). The primitive exposes `questionSync` and `createInterface`; the TS `Interface` class wraps the returned host-backed handle and forwards both readline methods (question/close/prompt/pause/resume/write/setPrompt/getPrompt) and EventEmitter methods (on/once/off/emit/…) via `this._inner.x(...)`. The existing `$ReadlineInterface` runtime class and `SharpTSReadlineInterface` stay intact as the host backing — no IL changes, just a registry re-key and a TS wrapper.

- `primitive:readline` added to `PrimitiveRegistry`, `PrimitiveModuleValues`, `BuiltInModuleTypes.GetPrimitiveTypes` (reusing `GetReadlineModuleTypes` — primitive surface matches module surface). ✅
- `ReadlinePrimitiveInterpreter` and `ReadlinePrimitiveEmitter` replace the `Module` variants; `ReadlinePrimitiveEmitter` registers under `primitive:readline` only. `RegisterBuiltInModuleMethod("readline", ...)` calls dropped from `RuntimeEmitter.ReadlineHelpers.cs` (tty pattern — CJS `require('readline')` flows through the standard ESM→CJS namespace-object path). ✅
- `stdlib/node/readline.ts` — `questionSync` re-export, `Interface` class with full method surface (readline + EventEmitter), and `createInterface(options)` factory. Method-chaining methods (`close`/`pause`/`resume`/event setters) return `this` (TS wrapper instance) rather than the inner, matching Node's chaining semantics. ✅
- Removed `"readline"` from `BuiltInModuleRegistry`, `BuiltInModuleValues` (dispatch + `HasInterpreterSupport`), and `BuiltInModuleTypes.GetModuleTypes`. Deleted `ReadlineModuleInterpreter.cs` and `ReadlineModuleEmitter.cs`. ✅

**Test gate:** 10071 pass, 16 skipped (pre-existing), 0 failed. All 30 `ReadlineModuleTests` pass in both modes.

**Not migrated — scope note.** The remaining built-in modules (`fs`, `fs/promises`, `crypto`, `stream`, `net`, `tls`, `http`, `https`, `child_process`, `dns`, `dns/promises`, `dgram`, `cluster`, `worker_threads`, `vm`, `zlib`, `stream/promises`, `stream/web`, `buffer`) are intentionally staying C#-backed. Their surface is primarily host-I/O marshalling — a TS facade would relocate rather than delete work, adding a TS↔C# boundary on every call while inviting compiler-bug workarounds like those accumulated in `url.ts` and `process.ts`. The triple-sync risk is real for these too, but the payoff curve inverts: each migration would move ~500–5000 lines of C# into an equally-heavy primitive plus a TS shell, for a net code increase. The plan's "Keep hot-path modules in C#. Migration is opportunistic" stance applies here — this is the intentional endpoint, not a loose end.

2. **Composite after leaves**: `stream`, `fs`, `fs/promises`, `http`, `https`, `net`, `tls`
